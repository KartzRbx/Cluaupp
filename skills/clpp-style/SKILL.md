...
Apply this whenever writing or refactoring CL++.
...
Define methods in the same order as the header. `void init()` only calls binders:
...
CL++ does **not** count uses inside a nested `func () { }` as uses of the outer parameters. Mention every parameter in the method body (`guard`) before the lambda.

Flare handlers must match the packet arity. If a field is unused, put it in the `guard` — do not drop it from the signature.
