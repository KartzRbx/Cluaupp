...
description: Design server-authoritative systems with Hive ECS and Flare packets. Use when editing .hive, .flare, CombatServer, CombatClient, hitboxes, AttackIntent, or when the user mentions Hive, Flare, ECS, snapshot, or hitbox.
---

# Hive + Flare

Server writes Hive columns and owns gameplay. Client sends intent and plays visuals.
...
Do not edit generated `.clh`. Change the schema and rebuild.

## Authority

- One `Schema.New()` on the **server** struct.
...
- Client never `Set`s Hive columns. Client stores replica fields from server packets.
...
Client `init`: construct the struct, init fields, bind input, `CombatReady.FireServer()`.
Server `OnReady`: ensure fighter row, `CombatHello.Fire(player, entity, health)`.

## Includes

Server header may include `"ReplicatedStorage/Shared/Net/Net.clh"` and the generated Hive header.
...
After `cluaupp build`, client Luau must read:
