...
exports.diffProfiles = diffProfiles;
...
function diffProps(owner, before, after, out) {
    ...
    for (const [name, prop] of aMap) {
        if (!bMap.has(name)) {
            ...
            if ((0, resolver_js_1.typeToClpp)(old.valueType) !== (0, resolver_js_1.typeToClpp)(prop.valueType)) {
                out.push({
                    kind: "CHANGED_PROPERTY_TYPE",
                    symbol: `${owner}.${name}`,
                    oldValue: (0, resolver_js_1.typeToClpp)(old.valueType),
                    newValue: (0, resolver_js_1.typeToClpp)(prop.valueType),
                });
            }
        }
    }
...
function diffMethods(owner, before, after, out) {
    const key = (m) => `${m.name}#${m.overloadIndex}`;
...
function diffEvents(owner, before, after, out) {
    ...
    for (const [name, ev] of aMap) {
        if (!bMap.has(name)) {
            ...
            const oldSig = bMap.get(name).parameters.map((p) => (0, resolver_js_1.typeToClpp)(p.type)).join(",");
            const newSig = ev.parameters.map((p) => (0, resolver_js_1.typeToClpp)(p.type)).join(",");
            if (oldSig !== newSig) {
                out.push({
