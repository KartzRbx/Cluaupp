...
function emitPaths(rel: string, stem: string) {
...
export function isNativeSchemaFile(fileName: string): boolean {
	const ext = path.extname(fileName).toLowerCase();
	return [".mint", ".bloom", ".helm", ".shift", ".hive", ".axiom"].includes(ext);
}

export function collectNativeSchemaFiles(dir: string, files: string[] = []): string[] {
...
function parseOptName(source: string, file: string, defaultName: string): { name: string; lines: string[] } {
...
function compileMint(source: string, file: string, defaultName: string, dir: string): NativeEmit {
	const { name, lines } = parseOptName(source, file, defaultName);
	if (lines.length === 0) {
		fail(file, 1, "schema is empty — add a formatter");
	}
	const formatters: Array<{ name: string; kind: string; suffixes: string[]; fraction: number; grouping: boolean }> = [];
	for (const tagged of lines) {
		const [lineNoRaw, line] = tagged.split("\t");
		const lineNo = Number(lineNoRaw);
		const match = line.match(
			new RegExp(
				`^formatter\\s+(${IDENT})\\s+(compact|group|integer|percent|scientific)(?:\\s*\\((.*)\\))?(?:\\s+fraction\\s+(\\d+))?(?:\\s+grouping\\s+(on|off))?\\s*$`,
				"i",
			),
		);
		if (!match) {
...
function parseBloomGradient(body: string, file: string, lineNo: number): { stops: Array<{ t: number; hex: string }>; speed: number } {
...
function compileBloom(source: string, file: string, defaultName: string): NativeEmit {
	const { name, lines } = parseOptName(source, file, defaultName);
	if (lines.length === 0) {
		fail(file, 1, "schema is empty — add a preset");
	}
	const presets: Array<{ name: string; stops: Array<{ t: number; hex: string }>; speed: number }> = [];
	for (let i = 0; i < lines.length; i++) {
		const [lineNoRaw, line] = lines[i].split("\t");
		const lineNo = Number(lineNoRaw);
		const inline = line.match(
			new RegExp(`^preset\\s+(${IDENT})\\s+gradient((?:\\s+[0-9.]+\\s+#[0-9A-Fa-f]{6})+)(?:\\s+speed\\s+([0-9.]+))?\\s*$`),
		);
		if (inline) {
			presets.push({
				name: inline[1],
...
function compileHelm(source: string, file: string, defaultName: string): NativeEmit {
	const { name, lines } = parseOptName(source, file, defaultName);
	if (lines.length === 0) {
		fail(file, 1, "schema is empty — add a command");
	}
	const commands: Array<{ name: string; args: Array<{ type: string; name: string }>; permission: string }> = [];
	let id = 1;
	for (const tagged of lines) {
		const [lineNoRaw, line] = tagged.split("\t");
		const lineNo = Number(lineNoRaw);
		const match = line.match(
			new RegExp(`^command\\s+(${IDENT})\\s*\\((.*)\\)\\s*(?:permission\\s+(${IDENT}))?\\s*$`, "i"),
		);
		if (!match) {
...
function compileShift(source: string, file: string, defaultName: string): NativeEmit {
	const { name, lines } = parseOptName(source, file, defaultName);
	const states: string[] = [];
	const parents: Record<string, string> = {};
	const transitions: Array<{ from: string; to: string; guard: string }> = [];
	for (const tagged of lines) {
		const [lineNoRaw, line] = tagged.split("\t");
		const lineNo = Number(lineNoRaw);
...
function compileHive(source: string, file: string, defaultName: string): NativeEmit {
	const { name, lines } = parseOptName(source, file, defaultName);
	const components: Array<{ name: string; fields: Array<{ type: string; name: string }> }> = [];
	for (const tagged of lines) {
		const [lineNoRaw, line] = tagged.split("\t");
		const lineNo = Number(lineNoRaw);
...
`;
	const rows = components
		.map(
			(item, index) =>
				`\t${item.name} = { Id = ${index + 1}, Fields = { ${item.fields
...
export function compileNativeSchemaFile(file: string, srcDir: string): NativeEmit {
...
export const compileMintSchema = compileMint;
export const compileBloomSchema = compileBloom;
export const compileHelmSchema = compileHelm;
export const compileShiftSchema = compileShift;
export const compileHiveSchema = compileHive;
export const compileAxiomSchema = compileAxiom;
