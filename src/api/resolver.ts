...
import type { CanonicalType, DefaultKind, ParameterDefinition } from "./model.js";

export type RobloxTypeRef = {
	Category?: string;
	Name?: string;
	Optional?: boolean;
};
...
export function hashBuffer(data: string | Buffer): string {
	return createHash("sha256").update(data).digest("hex");
}

export function mapRobloxType(ref: RobloxTypeRef | string | undefined | null): CanonicalType {
	if (!ref) {
		return { kind: "Unknown" };
	}
...
function stripOptionalMarker(type: CanonicalType): CanonicalType {
...
function mapNamedType(rawName: string): CanonicalType {
...
export function mapParameter(
	param: { Name?: string; Type?: RobloxTypeRef; Default?: string },
	index: number,
): ParameterDefinition {
...
function classifyDefault(value: string | undefined): DefaultKind | undefined {
	if (value === undefined) {
		return undefined;
	}
...
export function typeToClpp(type: CanonicalType): string {
	switch (type.kind) {
		case "Primitive":
...
export function typeToLuau(type: CanonicalType): string {
	switch (type.kind) {
		case "Primitive":
...
export function eventSignalType(parameters: ParameterDefinition[]): CanonicalType {
	return {
		kind: "Datatype",
