...
function parseFields(list: string, file: string, line: number): FlareField[] {
	const trimmed = list.trim();
	if (!trimmed) {
		return [];
	}
	const fields: FlareField[] = [];
...
function parseSide(raw: string, file: string, line: number): FlareSide {
