...
import type { RunContext } from "./roblox.js";
...
export interface CapabilityRule {
	id: string;
	/** Human message for diagnostics */
	message: string;
	/** Regex source matched against comment-stripped CL++ lines */
	pattern: string;
	/** Contexts where this pattern is illegal */
	forbiddenIn: RunContext[];
...
export function builtinCapabilityProfile(): CapabilityProfile {
	return {
...
export function capabilitiesForContext(context: RunContext): CapabilityId[] {
	switch (context) {
		case "Client":
