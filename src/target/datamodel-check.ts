...
import { lookupChild } from "./datamodel.js";

export interface DatamodelViolation extends CompileDiagnostic {
	code: string;
	pathHint?: string;
}
...
const CHAIN =
	/(?:GetService\s*<\s*([A-Za-z_]\w*)\s*>\s*\(\s*\)|GetService\s*\(\s*"([A-Za-z_]\w*)"\s*\)|(?:\bgame\s*\.\s*)?([A-Za-z_]\w*))((?:\s*\.\s*(?:WaitForChild|FindFirstChild|FindFirstChildOfClass|FindFirstChildWhichIsA)\s*\(\s*"[^"]+"\s*(?:,[^)]*)?\))+)/g;

const CHILD_NAME = /\.\s*(?:WaitForChild|FindFirstChild|FindFirstChildOfClass|FindFirstChildWhichIsA)\s*\(\s*"([^"]+)"/g;

const BIND =
	/(?:auto|Instance|[A-Za-z_]\w*)\s+([A-Za-z_]\w*)\s*=\s*(?:GetService\s*<\s*([A-Za-z_]\w*)\s*>\s*\(\s*\)|GetService\s*\(\s*"([A-Za-z_]\w*)"\s*\)|([A-Za-z_]\w*))((?:\s*\.\s*(?:WaitForChild|FindFirstChild|FindFirstChildOfClass|FindFirstChildWhichIsA)\s*\(\s*"[^"]+"\s*(?:,[^)]*)?\))*)/g;
...
function serviceRoot(profile: DatamodelProfile, serviceName: string): DatamodelNode | null {
	return lookupChild(profile.root, serviceName) || null;
}

function extractChildNames(chainTail: string): string[] {
	const names: string[] = [];
...
function walkFrom(profile: DatamodelProfile, rootName: string, names: string[]): { node: DatamodelNode; walked: string; missing?: string } | null {
	let node = serviceRoot(profile, rootName) || profile.byPath[rootName] || null;
	if (!node) return null;
	let walked = rootName;
	for (const childName of names) {
		const child = lookupChild(node, childName);
		if (!child) {
...
function buildVarEnv(clean: string, profile: DatamodelProfile): Map<string, DatamodelNode> {
	const env = new Map<string, DatamodelNode>();
	BIND.lastIndex = 0;
	let m: RegExpExecArray | null;
...
function validateChain(
	...
	index: number,
	fileHint?: string,
	env?: Map<string, DatamodelNode>,
): DatamodelViolation[] {
	const out: DatamodelViolation[] = [];
...
export function checkDatamodelPaths(source: string, profile: DatamodelProfile): DatamodelViolation[] {
	const clean = stripComments(source);
	const env = buildVarEnv(clean, profile);
	const violations: DatamodelViolation[] = [];
	CHAIN.lastIndex = 0;
	let m: RegExpExecArray | null;
	while ((m = CHAIN.exec(clean))) {
		const rootName = m[1] || m[2] || m[3];
		const names = extractChildNames(m[4]);
		if (!rootName || !names.length) continue;
		violations.push(...validateChain(profile, rootName, names, source, m.index, undefined, env));
	}
	return violations;
}

export function assertDatamodelSafe(source: string, profile: DatamodelProfile): void {
...
export function formatDatamodelViolations(file: string, violations: DatamodelViolation[]): string {
