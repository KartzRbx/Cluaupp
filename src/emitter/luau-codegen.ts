import type { TranspilerState } from "../types.js";

const GET_SERVICE = /^const\s+([A-Za-z_][\w]*)\s*=\s*game:GetService\("([A-Za-z_][\w]*)"\)\s*$/;
const REQUIRE_LINE = /^const\s+([A-Za-z_][\w]*)\s*=\s*require\((.+)\)\s*$/;

export class LuauCodeEmitter {
	public static emptyState(strict = false): TranspilerState {
		return {
	...
	public static emit(state: TranspilerState): string {
		const output: string[] = [];
		if (state.strict) {
			output.push("--!strict");
		}
	...
	public static merge(state: TranspilerState, compiledLuau: string): string {
...
function harvestPreamble(luau: string): {
	header: string[];
	services: Set<string>;
...
function bodyHasDecl(body: string, line: string, pattern: RegExp): boolean {
