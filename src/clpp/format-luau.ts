...
type Carry = {
	long: "none" | "string" | "comment";
	eq: number;
};

function longClose(eq: number): string {
	return `]${"=".repeat(eq)}]`;
}

function longOpenEq(text: string, index: number): number | null {
	if (text[index] !== "[") {
		return null;
	}
	let i = index + 1;
...
function skipLong(text: string, start: number, eq: number): { next: number; closed: boolean } {
...
function identAt(text: string, from: number): string {
	let j = from;
...
function scanDelta(text: string, carry: Carry): { delta: number; carry: Carry } {
	let i = 0;
	let delta = 0;
	let long = carry.long;
	let eq = carry.eq;

	const consumeLong = (kind: "string" | "comment", depth: number, from: number): boolean => {
...
export function formatLuauRoblox(source: string): string {
	const lines = String(source).split(/\r?\n/);
	const out: string[] = [];
	let indent = 0;
	let carry: Carry = { long: "none", eq: 0 };
...
function spaceScopes(lines: string[]): string[] {
	const out: string[] = [];
...
function compactTableFields(lines: string[]): string[] {
	const out: string[] = [];
	for (let i = 0; i < lines.length; i += 1) {
		const line = lines[i] || "";
		if (line.trim() !== "") {
