...
function fileStem(fileName) {
	return path.basename(String(fileName || "module")).replace(/\.(h|hpp|hh|cpp|cc|cxx|c)$/i, "");
}
...
function methodType(fn, owner) {
	...
	const ret = luauType(fn.returnType);
...
function fieldType(field, owners) {
	...
	return luauType(field.valueType) || "any";
}
...
function emitHeaderModule(ast, options = {}) {
	...
	for (const decl of shape.consts) {
		const typeAnn = luauType(decl.valueType);
