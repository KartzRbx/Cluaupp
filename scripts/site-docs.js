...
const { uiPageInner } = require("./site-docs-ui");

function createDocsPages({ codePair, preCode, escapeHtml, mappingTable }) {
	const pages = [];
	const page = (id, file, title, group, inner) => {
	...
	page(
		...
		"Introduction",
		"Start",
		`<p class="muted">Cluaupp is a C++ subset that compiles to readable Luau for Roblox. It is the same idea as <a href="https://roblox-ts.com">roblox-ts</a>: a familiar syntax, a restricted language, Studio-ready output.</p>
<p>You write <code>.cpp</code> / <code>.h</code>. Cluaupp emits Luau (<code>local</code>, <code>const</code>, <code>const function</code>). Editor completion is <strong>clangd</strong> reading <code>#include &lt;cluaupp/roblox.hpp&gt;</code>. There is no WASM, no Emscripten, no <code>lua_call</code>.</p>
<p>This handbook is the language. Every construct the CLI accepts is documented here — strings, functions, scopes, structs, callbacks, singletons, includes, and the commands that compile them. Engine member lists stay on <a href="https://create.roblox.com/docs/reference/engine">create.roblox.com</a>.</p>
<h2>A real script</h2>
<p><code>void init()</code> is the entry point. Filename tags pick Script vs LocalScript. Methods live on a <code>struct</code> in a sibling header.</p>
${codePair(
...
<h2>What Cluaupp is not</h2>
<ul>
<li>Not ISO C++. No <code>std::</code>, no macros, no pointer arithmetic, no custom metatables from <code>class</code> bodies.</li>
<li>Not a second IntelliSense engine. clangd completes; the compiler translates.</li>
<li>Not the Roblox API dump. Engine types come from <code>roblox.hpp</code>; official reference stays on Creator Hub.</li>
</ul>
<p>Start with <a href="setup.html">Setup</a>, keep <a href="reference.html">Language reference</a> open, then write a type in <a href="structs.html">structs and methods</a>.</p>`,
	);
...
<p><code>std::</code> (except type aliases the mapper knows), macros besides pragmas, C-style <code>for</code>, <code>++ -- +=</code>, <code>continue</code>, <code>do/while</code>, ternary <code>? :</code>, <code>int&amp;</code> references, <code>*p</code>, pointer arithmetic, <code>delete</code>, overloading, general templates, JSX, custom <code>class</code> vtables, <code>goto</code>, <code>try/catch</code>. Full contrast: <a href="vs-cpp.html">Syntax vs C++</a>.</p>`,
	...
	page(
		...
		"Types",
		"Language",
		`<p class="muted">C++ types are a contract. Luau sees the mapped type under <code>--!strict</code>.</p>
<table>
...
<p>Arrays, optionals, nested Templates: <a href="advanced-types.html">Advanced types</a>.</p>`,
	);
...
<p><code>const</code>, <code>constexpr</code>, <code>static</code>, <code>inline</code> may precede a declaration. <code>const</code> / <code>constexpr</code> mark Luau <code>const</code>. <code>static</code> / <code>inline</code> do not emit extra Luau. <code>volatile</code> is stripped from type names in the Tree-sitter mapper only.</p>`,
	...
	page(
		...
		"Functions",
		"Language",
		`<p class="muted">Only functions with a body are emitted from a <code>.cpp</code>. Prototypes in a header become type fields. There is no overloading — one name, one emit.</p>
...
<p>Pass a free function by name: <code>players-&gt;PlayerAdded.Connect(OnPlayer)</code>. Pass a method of the current struct and Cluaupp binds <code>self</code>: <code>function(...) self:OnPlayer(...) end</code>. Lambdas: <a href="events.html">Events and callbacks</a>.</p>`,
	);
...
<p>Use <code>string_concat</code> when more than two pieces, or when a piece is not a <code>.Name</code> / literal (so <code>+</code> would stay arithmetic). Logging is not concat — use <a href="logging.html">print / cout</a>.</p>`,
	...
	page(
		...
		"print and cout",
		"Language",
		`<p class="muted">No <code>&lt;iostream&gt;</code>. Logging is Roblox <code>print</code> / <code>warn</code> / <code>error</code>. C++ streams flatten to those globals.</p>
${codePair(
	`print("ok");
warn("careful");
error("fail");
...
<p>The first argument of <code>new Class(parent)</code> becomes <code>.Parent</code> only for Roblox Instance classes. Datatypes use <code>Vector3(...)</code>, not <code>new</code>. Always test <code>FindFirstChild</code> before using the result. Official class list: <a href="https://create.roblox.com/docs/reference/engine/classes">create.roblox.com</a>.</p>`,
	...
	page(
		...
		"Roblox",
		`<p class="muted">Signals use <code>.Connect</code>. Callbacks may be named functions or lambdas. There is no <code>std::function</code> type in emit — a lambda is a Luau <code>function</code>.</p>
${codePair(
	`players->PlayerAdded.Connect([&](Player* playerEntered) {
	leaderstatsServer.PlayerEntered(playerEntered);
});
...
function docsNav(pages, currentId) {
	const groups = [];
	...
	let html = `<nav class="docs-nav" aria-label="Docs">`;
	for (const group of groups) {
		html += `<h2>${group.name}</h2><ul>`;
		for (const page of group.items) {
			const current = page.id === currentId ? ' aria-current="page"' : "";
			html += `<li><a href="${page.file}"${current}>${page.title}</a></li>`;
		}
		html += `</ul>`;
	}
	html += `</nav>`;
...
module.exports = { createDocsPages, docsNav };
