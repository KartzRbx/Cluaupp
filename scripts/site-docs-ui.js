...
function uiPageInner({ codePair, preCode }) {
...
FusionState label = Fusion::Computed([]() {
...
ReactElement button = React::createElement("TextButton", {
	...
	.Text = "Buy",
	.Change = {
		.AbsoluteSize = [](GuiObject* rbx) {
			print(rbx->GetFullName());
		},
	},
	.Event = {
		.Activated = [](GuiObject* rbx) {
			print("clicked");
		},
	},
});`,
...
void DrawEconomy() {
	if (Iris::Window("Economy")) {
		...
		if (Iris::Button("Print player")) {
			print(GetService<Players>()->LocalPlayer->Name);
		}
