---
name: clpp-language
description: Write valid CL++ (Cluaupp) that compiles to Luau. Always rebuild and fix unused-parameter and compile errors until cluaupp build is clean. Use when editing .clpp, .clh, .clp, .hive, .flare, .mint, .bloom, .helm, .shift, or .axiom files, compiling with cluaupp/clpp, or when the user mentions CL++, Cluaupp, Hive, or Flare.
---

# CL++ language

CL++ **0.8+** is C++-inspired source that `clpp` emits as Luau. Cluaupp orchestrates Rojo, includes, Hive, and Flare. There is no `int main()`. Scripts enter at `void init()`.
...
| `.clh` in `src/` | types/modules; prefer `import { } from` |
| `#include <clpp/...>` | IntelliSense only; emits `CluauppLibs` require |
| `.flare` `.hive` `.mint` `.bloom` `.helm` `.shift` `.axiom` | schemas; do not edit the generated `.clh` |

Entry scripts: `#pragma strict` then imports/includes then functions then `void init()`.
...
| `task::wait` / `Class::Method` | static, method definition, call on the struct table |
| `signal~>Connect(fn)` | Sweep janitor Connect |
| `"hi " .: name` | concat |
| `expr?` | Result try (not optional chaining) |
...
CLPP0202 = Result→plain assign. CLPP1101 = `?` on non-Result. CLPP1102 = non-exhaustive match.
...
Reserved Intent (do not teach as shipped): `requires`, `ensures`, `effect`, `newtype`, `unique`, `move`.

## Types and init

Prefer `auto` only for `FindFirstChild` / `FindFirstChildWhichIsA` results and Hive worlds (`auto World = PlayerECS.New()`). Method parameters and other struct fields always have a named type (`InputObject`, `Player`). Do not type fields as `HiveWorld` — that name is an IntelliSense stub and is not imported into the generated module. Initialize every field: `int coins = 0;`, never `int coins;`.
`null` is Luau `nil`.
`post` / `warn` / `report` → print / warn / error.

## Includes

- Prefer `import { Name } from "./X.clh"` for language deps
- Quoted path from **src root** for generated modules: `#include "ReplicatedStorage/Shared/Net/Net.clh"`
- Relative quoted `.clh` next to the script: `#include "CombatServer.clh"` (legado / same-stem splice)
